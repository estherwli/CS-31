...
int* maxwell(int* a, int* b) //returns the pointer that points to a larger number; if equal, return b pointer
{
	if (*a > *b)
		return a;
	else
...
void swap1(int* a, int* b) //swaps the addresses (actual value) of the pointers
{
	int* temp = a;
	a = b;
	b = temp;
...
void swap2(int* a, int* b) //swaps the integers that each pointer points to
{
	int temp = *a;
	*a = *b;
	*b = temp;
...
void mainSwap()
{
	...
	int* ptr = maxwell(array, &array[2]); //creates a pointer that points to array[0]
	*ptr = -1; //array[0] gets -1
	ptr += 2; //ptr now points to array[2]
	ptr[1] = 9; //array[3] gets 9
	*(array + 1) = 79; //array[1] gets 79
