...
int main()
{
	...
	//char doc[] = "a x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x b";
	char document[MAX_DOC_LENGTH + 1][MAX_DOC_LENGTH + 1];
	makeArray(doc, document);
	cout << rate(doc, x1, x2, e, 2);
}
...
void makeArray(const char document[], char doc[][MAX_DOC_LENGTH + 1]) {
	...
	for (int i = 0, j = 0; document[i] != '\0' && j < MAX_DOC_LENGTH + 1; j++) {
		while (document[i] == ' ' || !(isalpha(document[i])))//skips to the next non-space character
			i++;
		for (int k = 0; document[i] != '\0' && document[i] != ' ' && k < MAX_DOC_LENGTH + 1; i++) {
	...
	for (int i = 0; i < nPatterns; i++) {
		if (separation[i] < 0) {
			for (int j = i; j < nPatterns; j++) {
				if (separation[j] >= 0) {
					char temp1[MAX_WORD_LENGTH + 1]; //swaps elements of word1 array
					strcpy(temp1, word1[i]);
					strcpy(word1[i], word1[j]);
					strcpy(word1[j], temp1);

					char temp2[MAX_WORD_LENGTH + 1]; //swaps elements of word2 array
...
int rate(const char document[], const char word1[][MAX_WORD_LENGTH + 1], const char word2[][MAX_WORD_LENGTH + 1], const int separation[], int nPatterns) {
	...
	for (int i = 0; i < nPatterns; i++) {
		for (int j = 0; j < MAX_DOC_LENGTH + 1; j++) {
			if (strcmp(word1[i], doc[j]) == 0) { //if word1 matches a word in the document
				...
				for (int k = separation[i] + 1; k > 0; k--) { //finds the boundaries set by separation value
					if (j + k < MAX_DOC_LENGTH + 1 && strcmp(word2[i], doc[j + k]) == 0) { //if word2 comes after word1
						count++;
						break;
					}
